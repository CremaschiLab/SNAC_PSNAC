		...
		while n < nmax:
			...
			for ns in Number_of_Scenarios[fl]:
				...
				while snacIter < scount[fl]:
					NAC_Set = NAC_original(Uncertain_Parameters, Subset_Scenarios, Scenario_Outcomes)
				...
				sub_model_NAC_count = len(sub_model.NAC_Constraint) + len(sub_model.NAC2_Constraint) + len(sub_model.NAC3_Constraint)
				...
				print('sub_model_Max',sub_model_Max,'sub_model_AVG',sub_model_AVG)
				...
				save_file = 'SNAC' + str(fl) + "_" + str(ns) + "_" + str(nmax) + "_iterations"
				...
				if n == 0:
					...
					Header = "%-30s %-30s %-30s %-30s %-30s %-30s %-30s %-30s %-30s %-30s %-30s" %('SAA_ENPV',
					'SNAC_Time', 'SNAC_Model_Time', 'SNAC_Time(P)', 'SNAC_Model_Time(P)', 
					'SNAC_sol_Time', 'SNAC_sol_Time(P)', 
					'sub_model_AVG','sub_model_Max',
				...
				RESULTS = "%-30s %-30s %-30s %-30s %-30s %-30s %-30s %-30s %-30s %-30s %-30s" %(str(round(sub_model.Expected_NPV(), 4)),
					str(round(Snac_Total, 4)),  str(round(Model1TotalTime, 4)), str(round(Snac_process_time, 4)), str(round(model_process_time, 4)), 
					str(round(Model1Solve_TotalTime, 4)), str(model_solve_process_time),
					str(round(sub_model_AVG, 4)),str(round(sub_model_Max, 4)),
					str(NAC_AVG), str(NAC_MAX)
					)
