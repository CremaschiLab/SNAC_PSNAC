...
def NAC_Generator(Uncertain_Parameters, Scenarios, Scenario_Outcomes, opts=[], current_directory=''):
	...
	####################################################################
	### 					Generate sub sceanrio outcomes  
	####################################################################
	
	sub_scenario_outcomes = {}
	sub_scenario_outcomes = {i:Scenario_Outcomes[i] for i in Scenarios }
	...
	while k < len(cuts):
		if k == 1:
			clists = list(set(itertools.combinations(cuts,k)))
		else:
			clists.extend(list(set(itertools.combinations(cuts,k))))
		k += 1

	clists = [(i,) for i in clists]
	...
	results = [pool.apply_async(clist_manager, args=(sub_scenario_outcomes, Scenarios,Uncertain_Parameters, Parameters,cuts,i,sets,use_sets)) for i in clists]
	pool.close()
	pool.join()
	...
	# nmst = mst.union(*[set(results[i]) for i in clists])
