...
def deFP(prod,sg,time_step,resource_type,ss,resource_max,gammaL,gammaD,duration,trial_cost,resource_required, revenue_max,probability, success,last_time_step, last_trial, rev_run, open_rev, discounting_factor,answer):
	...
	def FP_Constraint_rule(model,i,j,t,s):
		indx = model.PRODUCT.index(i)
		jndx = model.STAGE_GATE.index(j)
		tndx = model.TIME_STEP.index(t)
		if model.answer[s][indx][jndx][tndx] == 1:
...
def SingleScenario(prod,sg,time_step,resource_type,resource_max,gammaL,gammaD,duration,trial_cost,resource_required, revenue_max,probability, success,last_time_step, last_trial, rev_run, open_rev, discounting_factor,outcome):
	...
	###Variables###
	model.Decision_X = Var(model.PRODUCT, model.STAGE_GATE, model.TIME_STEP, within=Binary)
	model.Decision_Y = Var(model.PRODUCT, model.STAGE_GATE, model.TIME_STEP, within=Binary)
	model.Decision_Z = Var(model.PRODUCT, model.STAGE_GATE, model.TIME_STEP, within=Binary)
	...
	###Calculate Costs associated with each trial
	def Costs(model):
		return model.Cst[s] == sum((1 - (0.025 * (t - 1))) * model.Trial_Cost[i,j] * model.Decision_X[i,j,t] for i in model.PRODUCT for j in model.STAGE_GATE for t in model.TIME_STEP )
	model.Total_Costs = Constraint(model.SCENARIO,rule=Costs)
	...
	def Net_Revenue2_rule(model):	
		return model.Rv2 == sum(-model.Success[i,s]*model.GammaD[i] * sum(model.Decision_Z[i,j,t,s] for j in model.STAGE_GATE if j > 1) for i in model.PRODUCT for t in model.TIME_STEP)
	model.Net_Revenue2 = Constraint(model.SCENARIO, rule=Net_Revenue2_rule)
	...
	def Free_Rev_1_rule(model):
		last_time = model.LastTimeStep
		return model.FRv1 == sum(model.Success[i] * model.Open_Revenue[i,j] * model.Discount_Factor[i,j]* model.Decision_Z[i,j,last_time,s] for i in model.PRODUCT for j in model.STAGE_GATE)
	...
	def Fail_Constraint_rule(model,i,j,t):
		if model.Outcome[s][prod.index(i)] + 1 < j:
			return model.Decision_X[i,j,t] == 0
		else:
			return Constraint.Skip
	model.Fail_Constraint = Constraint(model.PRODUCT,model.STAGE_GATE,model.TIME_STEP, rule=Fail_Constraint_rule)
	
	def Min_Start_Constraint_rule(model,i,j,t):
		sumtau = sum(model.Duration[i,k,t] for k in model.PRODUCT if k < j)
		if sumtau > t:
			return model.Decision_X[i,j,t] == 0
		else:
			return Constraint.Skip
	model.Min_Start_Constraint = Constraint(model.PRODUCT,model.STAGE_GATE,model.TIME_STEP, rule=Min_Start_Constraint_rule)
	return model
