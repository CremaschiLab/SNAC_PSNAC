...
def SAA(prod,sg,time_step,resource_type,ss,resource_max,gammaL,gammaD,duration,trial_cost,resource_required, revenue_max,probability, success,last_time_step, last_trial, rev_run, open_rev, discounting_factor, phiij, outcome):
	...
	model.phiijlist = [i for i in model.Phiij]
	...
	def ENPV(model):
		return sum((model.FRv[s] + model.Rv[s] - model.Cst[s]) for s in model.SCENARIO)
	model.Expected_NPV = Objective(rule=ENPV, sense=maximize)
	...
	def NAC2_Constraint_rule(model,i,j,t,s,sprime):
		if t > 1:
			...
			return Constraint.Skip
	model.NAC2_Constraint = Constraint(model.PRODUCT, model.STAGE_GATE, model.TIME_STEP, model.phiijlist, rule=NAC2_Constraint_rule)
	...
	def NAC3_Constraint_rule(model,i,j,t,s,sprime):
		if t >1:
			...
			return Constraint.Skip
	model.NAC3_Constraint = Constraint(model.PRODUCT, model.STAGE_GATE, model.TIME_STEP, model.phiijlist,rule=NAC3_Constraint_rule)
...
def SAA_full(prod,sg,time_step,resource_type,ss,resource_max,gammaL,gammaD,duration,trial_cost,resource_required, revenue_max,probability, success,last_time_step, last_trial, rev_run, open_rev, discounting_factor, phiij, outcome):
