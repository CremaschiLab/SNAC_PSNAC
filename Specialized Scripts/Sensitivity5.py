...
def main_func():
	...
	parms = ['nresources', 'ntrials', 'nplanning_horizon','cost','revenue','penalty1','penalty2','trial_dur', 'percent_contrained']
		...
		for p in parms:
			...
			if p == 'cost':
				...
				for m in parmmod:
					...
					ModKDA = KDA_Solve_Function(mmodeldata, newKDAOutput, [], ['max_solve'])
					...
					filewriterdata = [fl,p,m,ModKDA.ENPV,ENPV_Percent,comp_results,ModKDA.Problem_Count, ModKDA.Solve_Time, 0, 0]
					_write_file(filewriterdata,output_directory)
			
			elif p == 'trial_dur':
				
				for m in trialdurmod:
					
					### Make copy of modeldata
					mmodeldata = copy.deepcopy(model_data)
					
					### Update change parameter	
					for (i,j) in mmodeldata._data['trial_duration']:
							mmodeldata._data['trial_duration'][(i,j)] = m + mmodeldata._data['trial_cost'][(i,j)]
					...
					for r in mmodeldata._data['resource_type'][None]:
						...
						for i in mmodeldata._data['product'][None]:
							...
							for j in mmodeldata._data['trial'][None]:
								if mmodeldata._data['resource_requirement'][(i,j,r)] > rmax:
									rmax = mmodeldata._data['resource_requirement'][(i,j,r)]
							ror += rmax
						uc_resource[r] = ror
						
					for r in mmodeldata._data['resource_type'][None]:
						
						mmodeldata._data['max_resource'][(r,)] = mmodeldata._data['max_resource'][(r,)] + m * (uc_resource[r] - mmodeldata._data['max_resource'][(r,)])
