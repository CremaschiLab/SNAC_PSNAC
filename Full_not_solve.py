...
def main():
	...
	Number_of_Scenarios = {'modeldata_4.dat':[.75],'modeldata_10.dat':[.24],'modeldata3.dat':[.09375],'modeldata3_5.dat':[.192],'modeldata4_3.dat':[.148148148],'modeldata4.dat':[.5],'modeldata4_5.dat':[.0384,0.25],'modeldata5.dat':[.0625, 0.25],'modeldata6.dat':[.125, 0.5, 0.75]}
	...
	for fl in files:
		...
		while n < nmax:
			...
			for ns in Number_of_Scenarios[fl]:
				...
				pai = len(full_model.NAC3_Constraint)/(len(Product)*len(Trials)*(len(model_data._data['time_step'][None])-1))
				xxx = len(Product)*len(Trials)*(len(model_data._data['time_step'][None])-1)
				# print('pai',pai,xxx)
				...
				if n == 0:
					...
					Header = "%-30s %-30s %-30s %-30s %-30s" %('Full_scenario_pair',
				...
				RESULTS = "%-30s %-30s %-30s %-30s  %-30s" %(str(round(pai, 4)),
